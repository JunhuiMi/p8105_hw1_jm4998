...
The purpose of this homework is to work with data and present plots using `ggplot`.
...
The mean of x is `mean(df_2$x)`, the median of x is `median(df_2$x)`, and the  standard deviation of x is `sd(df_2$x)`.
The proportion of cases for which x + y > 1 is `sum(df_2$num)/500`
